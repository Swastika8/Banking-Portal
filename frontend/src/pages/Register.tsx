...
import { Sun, Moon, Lock, Mail, Landmark, User } from 'lucide-react';
...
export const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Registration failed. Administrator may have disabled registration.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-brand-navy dark:bg-black p-4 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="w-full max-w-md bg-white dark:bg-brand-matte-card border border-gray-200 dark:border-brand-matte-border p-8 rounded-2xl shadow-2xl transition-all duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="register-role" className="block text-xs font-semibold uppercase tracking-wider text-brand-navy/70 dark:text-brand-matte-text mb-1">
              Select Role
            </label>
            <select
              id="register-role"
              ...
              className="w-full px-4 py-2.5 bg-gray-50 dark:bg-black border border-gray-200 dark:border-brand-matte-border text-brand-navy dark:text-white rounded-xl focus:outline-none focus:ring-1 focus:ring-brand-gold focus:border-brand-gold transition-all"
            >
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-3 bg-gradient-to-r from-brand-gold-dark to-brand-gold hover:from-brand-gold hover:to-brand-gold-light text-brand-navy dark:text-brand-navy-dark font-bold rounded-xl shadow-lg transition-all transform hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50"
          >
